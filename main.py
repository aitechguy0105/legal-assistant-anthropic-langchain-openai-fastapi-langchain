...
def get_cutom_prompt(user_response, financial_data, response_data):
...
Your goal is to equip fathers with an informed, actionable understanding of the legal pathways available to them based solely on the details of their specific jurisdiction and co-parenting situation.
    ...
    ## USER RESPONSE: 
    CONTEXT: This is the data the user has provided us directly.
    
    {user_response}
        
    ## RESPONSE DATA:
    CONTEXT: This data is the result of the previous USER RESPONSE that was input into the TaxSmart analysis tool. Always use this data to inform then respond to user questions.
...
def create_message_for_main_assistant(thread_id, content, role="user", data=[]):
    message = client.beta.threads.messages.create(thread_id=thread_id, role=role, content=content, timeout=60
                                                  )
    return message

# Run the assistantx
def run_assistant(thread_id, assistant_id):
    run = client.beta.threads.runs.create(thread_id=thread_id, assistant_id=assistant_id, timeout=60)
    return run
...
def execute_assistant(client, assistant_id, thread_id,custom_instruction=""):
...
def check_run_status(thread_id, run_id):
    run = client.beta.threads.runs.retrieve(thread_id=thread_id, run_id=run_id, timeout=60)
    return run

# List messages in a thread
def list_messages(thread_id):
    messages = client.beta.threads.messages.list(thread_id=thread_id, timeout=60, limit=100)
    return messages

def retrieve_assistant():
    return client.beta.assistants.retrieve(os.getenv("ASSISTANT_ID"), timeout=60)



def wait_for_run_completion(thread_id, run_id):
    while True:
        run = check_run_status(thread_id, run_id)
        if run.status == "completed":
            break
...
def initialize_convo_and_retrieve_convo(thread_id: str = None, assistant_id=os.getenv("ASSISTANT_ID")):
    print("thredsasddsaad_id", thread_id)
    if thread_id is None:
        thread_id = create_thread()
        assistant_id = assistant_id
        execute_assistant(client, assistant_id, thread_id,"")
        # wait_for_run_completion(thread_id, run.id)
    convo = list_messages(thread_id)
    return convo, thread_id

def add_message_and_retrieve_convo(thread_id, message, assistant_id=os.getenv("ASSISTANT_ID"), data=""):
    message = create_message(thread_id, message, "user")
    # run = run_assistant(thread_id, os.getenv("ASSISTANT_ID"))
    # wait_for_run_completion(thread_id, run.id)
    execute_assistant(client, assistant_id, thread_id, data)
    convo = list_messages(thread_id)
    return convo
...
def send_message_endpoint(message_request: MessageRequest, thread_id: str, role: str = "user"):
    message = create_message(thread_id, message_request.content, role)
    return {"message_id": message.id}

@app.post("/run-assistant/")
def run_assistant_endpoint(thread_id: str):
    assistant_id = os.getenv("ASSISTANT_ID")
    run = run_assistant(thread_id, assistant_id)
    return {"run_id": run.id}

@app.get("/list-messages/")
def list_messages_endpoint(thread_id: str):
    messages = list_messages(thread_id)
    return messages

@app.get("/initialize-convo")
def initialize_convo_endpoint(thread_id: str = Query(None, description="Optional thread ID to use if provided."), is_main_assistant: bool = False,user_id:str = None, is_delete: bool = False):
    if is_delete:
        supabase_client.table("personalfinance").delete().eq("userid", user_id).execute()

    assistant_id = os.getenv("ASSISTANT_ID")
    if is_main_assistant:
        assistant_id = os.getenv("MAIN_ASSISTANT_ID")

    convo, thread_id = initialize_convo_and_retrieve_convo(thread_id, assistant_id)
    return {"convo": convo, "thread_id": thread_id}
...
@app.post("/initialize-main-convo")
async def initialize_main_convo_endpoint(requestBody: dict):
    thread_id = requestBody["thread_id"]
    print("\n\n\n\n\n\n  ## THREAD ID:\n\n", thread_id)
    convo = ""
    main_thread = ""
    ...
    convo = initialize_convo_and_retrieve_convo(thread_id, os.getenv("MAIN_ASSISTANT_ID"))
    print("\n\n\n\n\n\n  ## THREAD ID:\n\n", convo)
    # list messages
...
@app.get("/add-message")    
async def add_message_endpoint(thread_id: str, content: str, user_id:str):
    ...
    if user_data.data:
        ...
        data = get_cutom_prompt(user_data.data, sheet2_data, sheet3_data)
        print("coming her ererer")
       
        convo = add_message_and_retrieve_convo(thread_id, content, os.getenv("MAIN_ASSISTANT_ID"), data)
        messages = list_messages(thread_id)
        messages = messages.json()
        new_messages = []
        first_message_content= []
        conversation_data = json.loads(messages)
        
    else:
        convo = add_message_and_retrieve_convo(thread_id, content, os.getenv("ASSISTANT_ID"), "")
        messages = list_messages(thread_id)
        messages = messages.json()
        new_messages = []
        first_message_content= []
    ...
    return {"convo": convo, "thread_id": thread_id, "outputJSON": new_messages, "main_thread": main_thread, "conversation_data": conversation_data}
# thread_owfe80dTr4nFQXJMXqrOlSTv

# create new assistant
@app.post("/create-assistant/")
def create_assistant_endpoint():
    assistant = client.beta.assistants.create(name="gpt-4 assistant with call to output_JSON",
